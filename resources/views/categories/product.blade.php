...
@php
    ...
    $images = is_array($item->multiple_image)
        ? $item->multiple_image
...
<div class="{{ $colSpan }} cursor-pointer overflow-hidden group rounded-lg border border-gray-200 relative">
    <a href="{{ route('product.article', ['slug' => $item->slug]) }}" class="block pointer-events-none">
        <div class="relative aspect-[5/6] overflow-hidden bg-white pointer-events-auto">
            @if($image1)
                <img
                    class="w-full h-full object-cover transition-opacity duration-500 absolute top-0 left-0 z-10 lazyload"
                    src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw=="
                    data-src="{{ $item->getThumbnail($image1, 'md') }}"
                    loading="lazy"
                    alt="{{ $item->name }}">
            @endif
            ...
            @if($image2)
                <img
                    class="w-full h-full object-cover transition-opacity duration-500 opacity-0 group-hover:opacity-100 absolute top-0 left-0 z-20 lazyload"
                    src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw=="
                    data-src="{{ $item->getThumbnail($image2, 'md') }}"
                    loading="lazy"
                    alt="{{ $item->name }} hover">
            @endif
        </div>
        ...
        <div class="lg:p-4 p-2 pointer-events-auto">
            ...
            <p class="text-red-600 w-full lg:text-[16px] text-sm">
                ...
                @else
                    ...
                    <span class="line-through text-sm text-gray-400 ml-2">{{ number_format($item->price, 0, '.', '.') }}đ</span>
                @endif
            </p>
        </div>
    </a>
