@php
    ...
    $headers = ['ID', 'Tên vai trò', 'Mô tả ngắn', 'Số thành viên', 'Ngày tạo', 'Hành động'];
@endphp
            ...
            @foreach($data as $row)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-4 text-sm">
                        <a href="{{ route('admin.roles.show', $row->id) }}" class="text-gray-700">
                            <p>{{ $row->name }}</p>
                        </a>
                    </td>
                    <td class="px-4 py-2 text-xs">{{ $row->description }}</td>
                    <td class="px-4 py-2 text-xs">{{ count($row->users) }}</td>
                    ...
                    <td>
                        <x-admin::form.dropdown
                            ...
                            :items="[
                                ['label' => 'Chỉnh sửa', 'href' => route('admin.roles.edit', $row->id)],
                                ['label' => 'Xóa', 'onclick' => 'deleteResource('.$row->id.', \'roles\')', 'class' => 'text-red-600']
                            ]"
                        >
                            <x-admin::form.button
                                type="button"
