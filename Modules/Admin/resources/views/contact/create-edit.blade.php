...
@section('title', isset($data) ? 'Chỉnh sửa Liên hệ' : 'Thêm mới Liên hệ')
...
@section('content')
    <div class="mb-6">
        <div class="relative group flex items-center">
            <a class="text-sm text-gray-500 dark:text-gray-800" href="{{ route('admin.contact.index') }}">
                Liên hệ
            </a>
    ...
    <form action="{{ isset($data) ? route('admin.contact.update', $data->id) : route('admin.contact.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @isset($data)
            @method('PUT')
        @endisset
        <div class="max-w-5xl space-y-2">
            <div class="flex items-center">
                <div class="block w-40 flex-none font-medium text-sm"> Tên Liên hệ </div>
                <div class="w-full">
                    <x-admin::form.input
                        name="title"
