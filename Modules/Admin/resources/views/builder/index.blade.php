@php
    ...
    $headers = ['ID', 'Tên builder', 'Slug', 'Mô tả', 'Ngày đăng', ''];
@endphp

@extends('admin::layouts.master')
@section('title', 'Danh sách Builder')

@section('content')
    <div class="flex items-center justify-between space-x-4 mb-4">
        <h1>Danh sách builder</h1>
        <a href="{{ route('admin.builder.create') }}" class="text-sm text-white font-medium bg-[#56a661] hover:shadow-md transition px-6 py-2.5 rounded-full cursor-pointer">
            Thêm mới
        </a>
    </div>

    <x-admin::table :headers="$headers" :rows="$rows">
        <x-slot:filters>
            <x-admin::filters>
                <x-slot:content>
                    <x-admin::form.select
                        ...
                        :options="array_merge(['' => 'Chọn trạng thái'], collect(Common::getStatuses())->mapWithKeys(fn($status) => [$status->value => $status->label()])->toArray())"
                    />
                    <x-admin::form.select
                        size="sm"
                        name="per_page"
            ...
            @foreach($rows as $row)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-4 text-sm">
                        <a href="{{ route('admin.builder.show', ['builder' => $row->id]) }}">{{ $row->name }}</a>
                    </td>
                    ...
                    <td>
                        <x-admin::form.dropdown
                            ...
                            :items="[
                                ['label' => 'Chỉnh sửa', 'href' => route('admin.builder.edit', $row->id)],
                                ['label' => 'Xóa', 'onclick' => 'deleteResource('.$row->id.', \'builder\')', 'class' => 'text-red-600']
                            ]"
                        >
                            <x-admin::form.button
                                type="button"
