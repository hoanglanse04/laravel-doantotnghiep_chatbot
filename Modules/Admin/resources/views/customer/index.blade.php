@php
    ...
    $headers = ['Thông tin', 'Liên kết', 'Giao diện', 'Trạng thái', 'Tác giả', 'Ngày tạo', 'Hành động'];
@endphp
            ...
            @foreach($data as $row)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-4 text-sm">
                        <a href="{{ route('admin.customer.show', $row->id) }}" class="text-gray-700">
                            <p>{{ $row->title }}</p>
                            <p class="text-xs">{{ $row->slug }}</p>
                        </a>
                    </td>
                    <td class="px-4 py-2 text-xs">{{ $row->slug }}</td>
                    <td class="px-4 py-2 text-xs">{{ $row->template ?: 'N/A' }}</td>
                    ...
                    <td>
                        <x-admin::form.dropdown
                            ...
                            :items="[
                                ['label' => 'Chỉnh sửa', 'href' => route('admin.user.edit', $row->id)],
                                ['label' => 'Xóa', 'onclick' => 'deleteResource('.$row->id.', \'user\')', 'class' => 'text-red-600']
                            ]"
                        >
                            <x-admin::form.button
                                type="button"
