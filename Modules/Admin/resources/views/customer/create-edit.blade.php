...
@section('title', isset($data) ? 'Chỉnh sửa trang đơn' : 'Thêm mới trang đơn')
...
@section('content')
    <div class="mb-6">
        <div class="relative group flex items-center">
            <a class="text-sm text-gray-500 dark:text-gray-800" href="{{ route('admin.customer.index') }}">
                Trang đơn
            </a>
    ...
    <form action="{{ isset($data) ? route('admin.customer.update', $data->id) : route('admin.customer.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @isset($data)
            @method('PUT')
        @endisset
        <div class="max-w-5xl space-y-2">
            <div class="flex items-center">
                <div class="block w-40 flex-none font-medium text-sm"> Tên trang đơn </div>
                <div class="w-full">
                    <x-admin::form.input
                        name="title"
            ...
            <div class="flex items-center">
                <div class="block w-40 flex-none font-medium text-sm"> Giao diện </div>
                <div class="w-full">
                    <x-admin::form.select
                        name="template"
                        :options="['' => 'Chọn một mục'] + $filenames"
                        :selected="old('template', $data->template ?? '')"
