@php
    ...
    $headers = ['Thông tin', 'Thuộc về', 'Trạng thái', 'Người tạo', 'Loại', 'Ngày tạo', 'Hành động'];
@endphp
            ...
            @foreach($data as $row)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-4 text-sm">
                        <a href="{{ route('admin.category.show', $row->id) }}" class="text-gray-700">
                            <p>{{ $row->breadcrumb_name }}</p>
                            <p class="text-xs">{{ $row->sku }}</p>
                        </a>
                    </td>
                    <td class="px-4 py-2 text-xs">{{ $row->parent->name ?? '' }}</td>
                    <td class="px-4 py-2 text-xs"><x-admin::chip :status="$row->status" /></td>
                    <td class="px-4 py-2 text-xs">{{ $row?->user?->name ?: "" }}</td>
                    <td class="px-4 py-2 text-xs">
                        {{ \App\Enums\ContentType::fromString($row->type)?->label() ?? 'Không xác định' }}
                    </td>
                    ...
                    <td>
                        <x-admin::form.dropdown
                            ...
                            :items="[
                                ['label' => 'Chỉnh sửa', 'href' => route('admin.category.edit', $row->id)],
                                ['label' => 'Xóa', 'onclick' => 'deleteResource('.$row->id.', \'category\')', 'class' => 'text-red-600']
                            ]"
                        >
                            <x-admin::form.button
                                type="button"
