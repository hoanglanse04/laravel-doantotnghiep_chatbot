...
@section('content')
    <div class="mb-6">
        <div class="relative group flex items-center">
            <a class="text-sm text-gray-500 dark:text-gray-800" href="{{ route('admin.category.index') }}">
                Chuyên mục
            </a>
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between text-xl">
            ...
            <a href="{{ route('admin.category.edit', ['category' => $data->id]) }}" class="text-sm text-white font-medium bg-[#56a661] hover:shadow-md transition px-6 py-2.5 rounded-full cursor-pointer">
                Chỉnh sửa
            </a>
        </div>
        ...
        <ul class="grid grid-cols-5 gap-6 ml-4">
            ...
            <li>
                <div class="uppercase font-semibold text-xs text-gray-600">Tên Chuyên mục</div>
                <span class="font-semibold mt-1 line-clamp-2">{{ $data->name }}</span>
            </li>
            <li>
                <div class="uppercase font-semibold text-xs text-gray-600">Loại</div>
                <span class="font-semibold mt-1 line-clamp-2">{{ \App\Enums\ContentType::fromString($row->type)?->label() ?? 'Không xác định' }}</span>
            </li>
            <li>
                <div class="uppercase font-semibold text-xs text-gray-600">Mô tả</div>
            ...
            <li>
                ...
                <span class="font-semibold mt-1 line-clamp-2">{{ $data->status }}</span>
            </li>
        </ul>

        <h2 class="font-semibold text-xl"> Ảnh Chuyên mục </h2>
        ...
        <h2 class="font-semibold text-xl"> Giới thiệu Chuyên mục & SEO </h2>
