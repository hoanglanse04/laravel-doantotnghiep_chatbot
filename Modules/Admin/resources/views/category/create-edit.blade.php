@php
    ...
    $typeOptions = collect(ContentType::cases())
        ->mapWithKeys(fn($case) => [$case->value => $case->label()])
        ->toArray();
@endphp
@extends('admin::layouts.master')
@section('title', isset($data) ? 'Chỉnh sửa Chuyên mục' : 'Thêm mới Chuyên mục')
...
@section('content')
    ...
    <form action="{{ isset($data) ? route('admin.category.update', $data->id) : route('admin.category.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @isset($data)
            @method('PUT')
        @endisset
        <div class="max-w-5xl space-y-2">
            <div class="flex items-center">
                <div class="block w-40 flex-none font-medium text-sm"> Tên Chuyên mục </div>
                <div class="w-full">
                    <x-admin::form.input
                        name="name"
            ...
            <div class="flex items-center">
                ...
                <div class="w-full">
                    <x-admin::form.select
                        ...
                        :selected="old('parent_id', $data->parent_id ?? '')"
                        placeholder="Thuộc về"
                        class="w-full"
                    />
                </div>
            </div>
            ...
            <div class="flex items-center">
                <div class="block w-40 flex-none font-medium text-sm">Loại</div>
                <div class="w-full">
                    <x-admin::form.select
                        name="type"
                        :options="$typeOptions"
                        :selected="old('type', $data->type ?? '')"
                        placeholder="Loại chuyên mục"
                        class="w-full"
                    />
                </div>
            </div>
