@php
    ...
    $headers = ['Thông tin', 'Số điện thoại', 'Giới tính', 'Trạng thái', 'Vai trò', 'Ngày tạo', 'Hành động'];
@endphp
            ...
            @foreach($data as $row)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-4 text-sm">
                        <a href="{{ route('admin.user.show', $row->id) }}" class="text-gray-700">
                            <p>{{ $row->name }}</p>
                            <p class="text-xs">{{ $row->email }}</p>
                        </a>
                    </td>
                    <td class="px-4 py-2 text-xs">{{ $row->phone }}</td>
                    <td class="px-4 py-2 text-xs"><x-admin::chip :status="$row->gender" /></td>
                    <td class="px-4 py-2 text-xs"><x-admin::chip :status="$row->status" /></td>
                    <td class="px-4 py-2 text-xs">{{ $row->role }}</td>
