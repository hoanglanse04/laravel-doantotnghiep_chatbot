@php
    ...
    $headers = ['Thông tin', 'Mô tả ngắn', 'Chuyên mục', 'Trạng thái', 'Tác giả', 'Ngày tạo', 'Hành động'];
@endphp
            ...
            @foreach($data as $row)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-4 text-sm">
                        <a href="{{ route('admin.post.show', $row->id) }}" class="text-gray-700">
                            <p>{{ $row->title }}</p>
                            <p class="text-xs">{{ $row->slug }}</p>
                        </a>
                    </td>
                    <td class="px-4 py-2 text-xs">{{ $row->description }}</td>
                    <td class="px-4 py-2 text-xs">{{ $row->category->name ?? '-' }}</td>
                    ...
                    <td>
                        <x-admin::form.dropdown
                            ...
                            :items="[
                                ['label' => 'Chỉnh sửa', 'href' => route('admin.post.edit', $row->id)],
                                ['label' => 'Xóa', 'onclick' => 'deleteResource('.$row->id.', \'post\')', 'class' => 'text-red-600']
                            ]"
                        >
                            <x-admin::form.button
                                type="button"
