@php
    ...
    $headers = ['Thông tin', 'Giá bán', 'Chuyên mục', 'Trạng thái', 'Tác giả', 'Ngày tạo', 'Hành động'];
@endphp
            ...
            @foreach($data as $row)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-sm">
                        <a href="{{ route('admin.product.show', $row->id) }}" class="text-gray-700 flex items-center space-x-2">
                            <input type="image" src="{{ $row->image }}" alt="Ảnh {{ $row->id }}" class="w-10 h-10 rounded-full">
                            <div>
                                <p>{{ $row->name }}</p>
                                <p class="text-xs">{{ $row->sku }}</p>
                            </div>
                        </a>
                    </td>
                    <td class="px-4 py-2 text-xs">{{ number_format($row->final_price, 0, ',', '.') }}</td>
                    <td class="px-4 py-2 text-xs">{{ $row->category->name ?? "Chưa có chuyên mục" }}</td>
                    <td class="px-4 py-2 text-xs"><x-admin::chip :status="$row->status" /></td>

                    <td class="px-4 py-2 text-xs">{{ $row->user->name }}</td>
                    ...
                    <td>
                        <x-admin::form.dropdown
                            ...
                            :items="[
                                ['label' => 'Chỉnh sửa', 'href' => route('admin.product.edit', $row->id)],
                                ['label' => 'Xóa', 'onclick' => 'deleteResource('.$row->id.', \'product\')', 'class' => 'text-red-600']
                            ]"
                        >
                            <x-admin::form.button
                                type="button"
