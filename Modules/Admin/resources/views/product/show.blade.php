...
@section('content')
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between text-xl">
            ...
            <a class="text-sm text-white font-medium bg-[#56a661] hover:shadow-md transition px-6 py-2.5 rounded-full cursor-pointer" href="{{ route('admin.product.edit', ['product' => $data->id]) }}">
                Chỉnh sửa
            </a>
        </div>
        ...
        <ul class="grid grid-cols-5 gap-6 ml-4">
            ...
            <li>
                <div class="uppercase font-semibold text-xs text-gray-600">Tên sản phẩm</div>
                <span class="font-semibold mt-1 line-clamp-2">{{ $data->name }}</span>
            </li>
            <li>
                <div class="uppercase font-semibold text-xs text-gray-600">Mã sản phẩm</div>
                <span class="font-semibold mt-1 line-clamp-2">{{ $data->sku }}</span>
            </li>
            <li>
                <div class="uppercase font-semibold text-xs text-gray-600">Chuyên mục</div>
                <span class="font-semibold mt-1 line-clamp-2">{{ $data?->category?->name }}</span>
            </li>
        </ul>

        <h2 class="font-semibold text-xl"> Giá bán & Khuyến mãi </h2>
        <ul class="grid grid-cols-5 gap-6 ml-4">
            <li>
                <div class="uppercase font-semibold text-xs text-gray-600">Giá bán</div>
                <span class="font-semibold mt-1 line-clamp-2">{{ number_format($data->price, 0, '.', '.') }}</span>
            </li>
            <li>
                <div class="uppercase font-semibold text-xs text-gray-600">Giá sau giảm</div>
                <span class="font-semibold mt-1 line-clamp-2">{{ number_format($data->discount_price, 0, '.', '.') }}</span>
            </li>
            <li>
                <div class="uppercase font-semibold text-xs text-gray-600">Phần trăm giảm</div>
                <span class="font-semibold mt-1 line-clamp-2">{{ number_format($data->discount_percentage, 0, '.', '.') }}</span>
            </li>
            <li>
                <div class="uppercase font-semibold text-xs text-gray-600">Giá cuối cùng sau giảm</div>
                <span class="font-semibold mt-1 line-clamp-2">{{ number_format($data->final_price, 0, '.', '.') }}</span>
            </li>
        </ul>

        <h2 class="font-semibold text-xl"> Ảnh sản phẩm </h2>
        <ul class="flex items-center gap-4 ml-4">
            @if (is_array($data->multiple_image) && count($data->multiple_image) > 0)
                @foreach($data->multiple_image as $image)
                    <div class="aspect-square">
                        <img class="object-cover rounded-md" src="{{ image($image) }}" width="100" height="100" />
                    </div>
                @endforeach
            @endif
        </ul>
